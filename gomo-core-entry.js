...
<main class="wrap">
  ...
  <div class="grid">
    ...
    <div class="card"><b id="conflicts">—</b><small>QG / rang à vérifier</small></div>
  ...
  <div class="note">Mode test : aucune donnée des autres sites n’est modifiée et aucun membre n’est supprimé automatiquement.<div class="detail" id="powerDiff">Écarts de puissance : —</div></div>
...
(() => {
  const nodes = {
    ...
    powerDiff: document.getElementById('powerDiff')
  };
  ...
  const criticalFlags = new Set(['hq_conflict','rank_conflict','ambiguous_name']);
  ...
  async function loadSources() {
    ...
    try {
      ...
      const criticalCount = members.filter((member) => (member.flags || []).some((flag) => criticalFlags.has(flag))).length;
      const powerDifferenceCount = members.filter((member) => (member.flags || []).includes('power_conflict')).length;
      ...
      nodes.conflicts.textContent = criticalCount;
      nodes.powerDiff.textContent = 'Écarts de puissance : ' + powerDifferenceCount + ' (informatifs, pas des erreurs)';
      ...
      nodes.rows.innerHTML = members.map((member) => {
        ...
        const criticalIssues = flags.filter((flag) => criticalFlags.has(flag));
        const hasPowerDifference = flags.includes('power_conflict') || flags.includes('hero_power_conflict');
        const stateClass = criticalIssues.length ? 'review' : hasPowerDifference ? 'info' : 'high';
        const stateText = criticalIssues.length ? '⚠ ' + criticalIssues.length : hasPowerDifference ? '≈' : '✓';
        return '<tr>' +
