// GoMo Central v20.11 — mission finale : raccourcis, Train, podiums et outils retirés.
...
function centralUpgrade() {
  if (window.__GOMO_CENTRAL_V2011__) return;
  window.__GOMO_CENTRAL_V2011__ = true;
  ...
  function addStyles() {
    if (document.getElementById("gomo-central-v2010-style")) return;
    const style = document.createElement("style");
    style.id = "gomo-central-v2010-style";
    style.textContent = `
      ...
      .royal-nav-grid{display:grid!important;grid-template-columns:repeat(6,minmax(0,1fr))!important;grid-auto-flow:row;gap:8px!important;overflow:visible;padding:11px!important}
      .royal-nav-button{min-width:0!important;min-height:112px;padding:7px 5px 9px!important;gap:6px!important;font-size:clamp(.68rem,2.3vw,.88rem)!important}.royal-nav-button img{width:min(72px,100%)!important;height:auto!important;max-width:72px!important;aspect-ratio:1;flex:0 0 auto;object-fit:cover}.royal-nav-button span{white-space:normal!important;overflow-wrap:anywhere;line-height:1.05;min-height:1.05em}
      ...
      #rankings{max-width:1220px}#rankings .rankings-hero{padding:18px 20px;border:1px solid rgba(225,161,34,.68);border-radius:24px;background:radial-gradient(circle at 92% 0,rgba(242,193,78,.13),transparent 38%),linear-gradient(145deg,#0d2034,#071320);box-shadow:0 18px 44px rgba(0,0,0,.32)}#rankings .rankings-hero h1{color:#f8d778;text-shadow:0 2px 0 #5a3607}#rankings .rankings-status{border-color:rgba(225,161,34,.35);background:rgba(8,27,45,.78)}#rankings .rankings-pill{border-color:rgba(242,193,78,.42);background:rgba(242,193,78,.1);color:#ffe6a1}#rankings .rankings-grid{gap:18px}#rankings .ranking-card{border-color:rgba(225,161,34,.68);border-radius:24px;background:linear-gradient(180deg,#0b2033,#071522);box-shadow:0 18px 42px rgba(0,0,0,.34),inset 0 0 0 1px rgba(255,226,148,.035)}#rankings .ranking-card__head{border-bottom-color:rgba(225,161,34,.43);background:linear-gradient(90deg,rgba(242,193,78,.11),transparent)}#rankings .ranking-card__head h2{color:#f8d778}#rankings .ranking-card__head small{color:#bdc9d6;opacity:1}
      .ranking-podium{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));align-items:end;gap:7px;padding:17px 9px 15px;background:radial-gradient(circle at 50% 0,rgba(242,193,78,.15),transparent 58%)}.ranking-podium__item{display:flex;min-width:0;min-height:174px;flex-direction:column;align-items:center;justify-content:flex-end;padding:10px 5px 12px;border:1px solid rgba(242,193,78,.4);border-radius:17px 17px 9px 9px;background:linear-gradient(180deg,rgba(28,65,94,.82),rgba(7,23,37,.98));text-align:center;box-shadow:0 10px 24px rgba(0,0,0,.29)}.ranking-podium__item--first{min-height:204px;border-color:#f3c752;background:linear-gradient(180deg,rgba(121,82,12,.5),rgba(12,36,54,.98));box-shadow:0 0 0 1px rgba(255,226,142,.14),0 12px 30px rgba(0,0,0,.38),0 0 24px rgba(242,193,78,.13)}.ranking-podium__item--second{min-height:184px}.ranking-podium__place{font-size:1.5rem;line-height:1}.ranking-avatar{display:grid;place-items:center;overflow:hidden;border-radius:50%;border:2px solid rgba(242,193,78,.82);background:radial-gradient(circle at 35% 25%,#356c91,#0b2438 68%);color:#ffe7a8;object-fit:cover;box-shadow:0 7px 18px rgba(0,0,0,.36);font-weight:900}.ranking-podium__avatar{width:58px;height:58px;margin:7px 0 6px}.ranking-podium__item--first .ranking-podium__avatar{width:70px;height:70px;border-color:#ffd76b}.ranking-podium__name{width:100%;color:#fff7dd;font-size:.78rem;line-height:1.08;overflow-wrap:anywhere}.ranking-podium__role{margin-top:3px;color:#aebdca;font-size:.66rem}.ranking-podium__score{margin-top:7px;color:#f7cc67;font-size:.76rem;font-variant-numeric:tabular-nums;overflow-wrap:anywhere}.ranking-list{padding:0 10px 12px!important}.ranking-row{grid-template-columns:31px 42px minmax(0,1fr) auto!important;gap:8px!important;padding:10px 7px!important;border-top:1px solid rgba(225,161,34,.16);border-radius:12px!important}.ranking-row:first-child{border-top:0}.ranking-row:nth-child(odd){background:rgba(255,255,255,.028)!important}.ranking-row__avatar{width:42px;height:42px;font-size:.7rem}.ranking-name strong{white-space:normal!important;overflow:visible!important;text-overflow:clip!important;overflow-wrap:anywhere;line-height:1.13}.ranking-name small{display:block;margin-top:3px}.ranking-score{max-width:108px;overflow-wrap:anywhere;font-size:.87rem;color:#fff1c5}.ranking-row--empty{grid-template-columns:1fr!important;text-align:center;color:#bdc9d6}
      .ranking-celebration{display:grid;grid-template-columns:58px minmax(0,1fr);align-items:center;gap:10px;margin:13px 12px 0;padding:9px 12px;border:1px solid rgba(242,193,78,.34);border-radius:16px;background:linear-gradient(90deg,rgba(242,193,78,.12),rgba(72,203,239,.06));color:#fff1c2;font-weight:800;line-height:1.25}.ranking-celebration img{width:58px;height:58px;object-fit:cover;object-position:center 18%;border-radius:14px;border:1px solid rgba(242,193,78,.58)}.ranking-celebration p{margin:0}
      ...
      @media(max-width:620px){.royal-nav-grid{grid-template-columns:none!important;grid-auto-flow:column;grid-auto-columns:78px;overflow-x:auto;overscroll-behavior-inline:contain;scroll-snap-type:inline proximity;scrollbar-width:none;padding:9px 8px!important;gap:6px!important}.royal-nav-grid::-webkit-scrollbar{display:none}.royal-nav-button{min-width:78px!important;min-height:104px;font-size:.7rem!important;scroll-snap-align:start}.royal-nav-button img{width:64px!important;height:64px!important;max-width:64px!important}.translated-train__hero{grid-template-columns:45% 55%;height:218px}.translated-train__hero-copy{padding:16px 12px 16px 0}.translated-train__hero-copy h1{font-size:clamp(1.35rem,7vw,2rem)}.translated-train__hero-copy p{font-size:.73rem}.translated-train__dates{margin-top:9px;padding:5px 6px;font-size:.75rem}.translated-train__scroll{padding:8px}.translated-train__table-frame{border-radius:14px}.translated-train th,.translated-train td{padding:8px 5px}.translated-train th{font-size:.68rem}.translated-train__person{align-items:flex-start;gap:4px;font-size:clamp(.61rem,2.45vw,.72rem)}.translated-train__name{white-space:normal;overflow:visible;text-overflow:clip;overflow-wrap:anywhere;letter-spacing:-.01em}.translated-train__avatar{flex-basis:32px;width:32px;height:32px;font-size:.55rem}.translated-train__avatar--pending{font-size:1.05rem}.translated-train__day{font-size:.82rem}.translated-train__day small{font-size:.65rem}.translated-train__pending{font-size:.66rem;white-space:normal}.translated-train th:nth-child(1),.translated-train td:nth-child(1){width:29%}.translated-train th:nth-child(2),.translated-train td:nth-child(2){width:35.5%}.translated-train th:nth-child(3),.translated-train td:nth-child(3){width:35.5%}.ranking-celebration{grid-template-columns:48px minmax(0,1fr);margin-inline:8px;padding:8px 9px;font-size:.78rem}.ranking-celebration img{width:48px;height:48px}}
      @media(max-width:480px){#rankings .rankings-hero{padding:15px}.ranking-podium{gap:5px;padding-inline:6px}.ranking-podium__item{min-height:164px;padding-inline:3px}.ranking-podium__item--second{min-height:174px}.ranking-podium__item--first{min-height:194px}.ranking-podium__avatar{width:50px;height:50px}.ranking-podium__item--first .ranking-podium__avatar{width:62px;height:62px}.ranking-podium__name{font-size:.69rem}.ranking-podium__role{font-size:.6rem}.ranking-podium__score{font-size:.67rem}.ranking-row{grid-template-columns:27px 38px minmax(0,1fr) auto!important;padding-inline:4px!important}.ranking-row__avatar{width:38px;height:38px}.ranking-score{max-width:88px;font-size:.76rem}}
      @media(max-width:390px){.translated-train__hero{grid-template-columns:43% 57%;height:202px}.translated-train__hero-copy .eyebrow{font-size:.58rem}.translated-train__hero-copy h1{font-size:1.28rem}.translated-train__hero-copy p{font-size:.66rem}.translated-train__dates{font-size:.66rem}.translated-train th,.translated-train td{padding-inline:3px}.translated-train__avatar{flex-basis:29px;width:29px;height:29px}.translated-train__person{gap:3px;font-size:.59rem}.translated-train__day{font-size:.74rem}.coach-intro.gomo-coach-with-mascot{grid-template-columns:72px minmax(0,1fr)}.gomo-coach-mascot{width:72px}}
    `;
  ...
  function memberProfile(name) {
    try {
      if (typeof window.GOMO_MEMBER_PROFILE === "function") return window.GOMO_MEMBER_PROFILE(name);
    } catch {}
    return null;
  }
  ...
  function safeMemberPhoto(value) {
    try {
      ...
      return /^\/assets\/member-profiles\/[0-9]+\.(?:jpg|jpeg|png|webp)$/i.test(url.pathname) ? url.href : "";
    } catch {
      return "";
    }
  }
  ...
  function avatarMarkup(name, pending = false) {
    ...
    const profile = memberProfile(name);
    const configured = profile?.photo || (window.GOMO_MEMBER_PHOTOS && window.GOMO_MEMBER_PHOTOS[name]);
    const photo = safeMemberPhoto(configured);
    if (photo) return `<img class="translated-train__avatar" src="${escapeMarkup(photo)}" alt="">`;
    return `<span class="translated-train__avatar" aria-hidden="true">${escapeMarkup(initials(name))}</span>`;
  }

  function personMarkup(name) {
    const fullName = memberProfile(name)?.name || name;
    return `<span class="translated-train__person">${avatarMarkup(fullName)}<span class="translated-train__name" title="${escapeMarkup(fullName)}">${escapeMarkup(fullName)}</span></span>`;
  }
  ...
  function removeRetiredCentralTools() {
    const retiredPages = new Set(["capture", "communication", "news"]);
    const currentPage = location.hash.replace(/^#/, "");
    const plannerAnalysis = currentPage === "capture"
      && new URLSearchParams(location.search).get("from") === "vs-planner";
    if (retiredPages.has(currentPage) && !plannerAnalysis) {
      if (typeof openPage === "function") openPage("home");
      else history.replaceState(null, "", `${location.pathname}${location.search}#home`);
    }

    document.querySelectorAll([
      '.nav-item[data-page="capture"]',
      '.nav-item[data-page="communication"]',
      '.nav-item[data-page="news"]',
      '#home .gomo-quick-card[data-go="capture"]',
      '#home .gomo-quick-card[data-go="communication"]',
      '#tools [data-go-card="capture"]',
      '#tools [data-go-card="communication"]',
      '#tools [data-go-card="news"]',
      '#gomo-r5fapper-panel [data-r5-go="traduction"]'
    ].join(",")).forEach((element) => element.remove());

    ["communication", "news", "messageDialog"].forEach((id) => {
      document.getElementById(id)?.remove();
    });
    if (!plannerAnalysis) document.getElementById("capture")?.remove();
  }
  ...
  function syncTools() {
    ...
    if (!grid) return;
    const assistantTitle = [...grid.querySelectorAll("h2")].find((title) =>
      /^GoMo Assistant$/i.test((title.textContent || "").trim())
    );
    const assistant = assistantTitle?.closest(".tool-card");
    if (assistant) {
      assistant.dataset.gomoAssistantCard = "1";
  ...
  function syncLinks() {
    ...
    document.querySelectorAll('a[href*="weeklyChampionsCard"]').forEach((link) => {
      link.setAttribute("href", `/?lang=${encodeURIComponent(lang)}#rankings`);
      link.dataset.go = "rankings";
      link.dataset.gomoRankingsOpen = "1";
  ...
  function loadRankingsDirectly() {
    ...
    const refresh = document.getElementById("rankingsRefresh");
    if (!refresh || refresh.dataset.gomoAutoLoad === "1") return;
    refresh.dataset.gomoAutoLoad = "1";
    setTimeout(() => {
      refresh.click();
      delete refresh.dataset.gomoAutoLoad;
    }, 0);
  }
  ...
  document.addEventListener("click", (event) => {
    ...
    const rankings = event.target.closest('[data-page="rankings"],[data-go="rankings"],[data-gomo-rankings-open],[data-gomo-rankings-card] button,[data-r5-go="classements"],a[href*="weeklyChampionsCard"]');
    if (rankings) {
      event.preventDefault();
      event.stopImmediatePropagation();
      if (typeof EXTERNAL_LINKS !== "undefined") {
      ...
      setTimeout(() => document.getElementById("rankingsRefresh")?.click(), 0);
      return;
    }
    const assistant = event.target.closest('[data-gomo-assistant-open],[data-r5-go="gomo-assistant"],[data-go="gomo-assistant"]');
    if (assistant) {
      event.preventDefault();
      event.stopImmediatePropagation();
  ...
  document.addEventListener("gomo:member-profiles", renderTrain);
...
function coachDictionaryUpgrade() {
  EVENT_COPY.de = [
    ...
    ["Allianzzug · VIP","Den Gold Express vorbereiten und Plätze fair verteilen.","Spende während der 4-stündigen Vorbereitung Verträge. Fünf Aktualisierungen beziehungsweise 25 Verträge ermöglichen den Gold Express.","Der Plan behält 4 R4/R5-Zugführer und 3 R3, die hauptsächlich nach den besten Ranglisten ohne erzwungene Rotation ausgewählt werden. Ein R3 kann in der folgenden Woche erneut gewählt werden, solange er zu den Besten gehört.","Vermeide doppelte Einträge im selben Plan und wechsle den Zugführer nicht ohne Planprüfung.","Handelsverträge, GoMo-Zugplan und VIP-Liste."],
  ...
  EVENT_COPY.ro = [
    ...
    ["Trenul alianței · VIP","Pregătește Gold Express și împarte locurile corect.","Donează contracte în cele 4 ore de pregătire. Cinci reîmprospătări, adică 25 de contracte, permit obținerea Gold Express.","Planul păstrează 4 Conductori R4/R5 și 3 R3 aleși în principal după cele mai bune clasamente, fără rotație forțată. Același R3 poate fi ales din nou săptămâna următoare cât timp rămâne printre cei mai buni.","Evită dublurile în același plan și nu schimba conductorul fără să verifici planificarea.","Contracte comerciale, planificarea Trenului GoMo și lista VIP."],
  ...
  EVENT_COPY.uk = [
    ...
    ["Потяг альянсу · VIP","Підготуйте Gold Express і справедливо розподіліть місця.","Протягом 4 годин підготовки жертвуйте контракти. П’ять оновлень, тобто 25 контрактів, дають шанс на Gold Express.","План зберігає 4 провідників R4/R5 і 3 R3, обраних насамперед за найкращими рейтингами без примусової ротації. Того самого R3 можна обрати знову наступного тижня, доки він серед найкращих.","Не допускайте дублювання в одному плані й не змінюйте провідника без перевірки розкладу.","Торгові контракти, розклад потяга GoMo та VIP-список."],
  ...
  EVENT_COPY.ko = [
    ...
    ["동맹 열차 · VIP","Gold Express를 준비하고 좌석을 공정하게 나누세요.","4시간 준비 동안 계약서를 기부하세요. 5회 갱신, 즉 25개 계약서로 Gold Express를 노릴 수 있습니다.","계획은 R4/R5 차장 4명과 강제 순환 없이 주간 순위를 우선으로 뽑은 R3 3명을 유지합니다. 계속 상위권이면 같은 R3가 다음 주에도 다시 선정될 수 있습니다.","한 계획 안의 중복을 방지하고 일정 확인 없이 차장을 바꾸지 마세요.","무역 계약서, GoMo 열차 일정, VIP 목록."],
  ...
  EVENT_COPY.hr = [
    ...
    ["Vlak saveza · VIP","Pripremi Gold Express i pravedno podijeli mjesta.","Tijekom 4 sata pripreme doniraj ugovore. Pet osvježavanja, odnosno 25 ugovora, omogućuje Gold Express.","Plan zadržava 4 R4/R5 voditelja i 3 R3 odabrana prvenstveno prema najboljim rezultatima, bez prisilne rotacije. Isti R3 može biti ponovno izabran sljedeći tjedan dok je među najboljima.","Spriječi duplikate u istom planu i ne mijenjaj voditelja bez provjere rasporeda.","Trgovački ugovori, raspored GoMo vlaka i VIP popis."],
...
class VersionedCentralScripts {
  element(element) {
    ...
    if (source.includes("assets/gomo-member-profiles.js")) element.setAttribute("src", `/assets/gomo-member-profiles.js?v=${VERSION}`);
...
class ShinyHeadBoot {
  element(element) {
    element.append(`<script src="/assets/gomo-member-profiles.js?v=${VERSION}"></script><script>;${clientFunctionCall(shinyBootstrap)};</script>`, { html: true });
  }
}
...
function rankingsRendererUpgrade() {
  return String.raw`
  const RANKING_CELEBRATION = {
    fr: "Bravo aux gagnants ❤️ 🎉 Merci à toutes et à tous pour votre participation !",
    de: "Glückwunsch an die Gewinner ❤️ 🎉 Danke an alle für eure Teilnahme!",
    en: "Congratulations to the winners ❤️ 🎉 Thank you all for taking part!",
    ro: "Felicitări câștigătorilor ❤️ 🎉 Vă mulțumim tuturor pentru participare!",
    uk: "Вітаємо переможців ❤️ 🎉 Дякуємо всім за участь!",
    ko: "우승자 여러분 축하합니다 ❤️ 🎉 참여해 주신 모든 분께 감사드립니다!",
    hr: "Čestitamo pobjednicima ❤️ 🎉 Hvala svima na sudjelovanju!",
    pt: "Parabéns aos vencedores ❤️ 🎉 Obrigado a todos pela participação!"
  };

  function safeRankingPhoto(value) {
    const photo = String(value || "");
    if (/^data:image\/(?:jpeg|jpg|png|webp);base64,[A-Za-z0-9+/=]+$/i.test(photo)) return photo;
    try {
      const url = new URL(photo, location.origin);
      if (url.origin === location.origin && /^\/assets\/member-profiles\/[0-9]+\.(?:jpg|jpeg|png|webp)$/i.test(url.pathname)) return url.href;
    } catch {}
    return "";
  }

  function rankingInitials(value) {
    return String(value || "").trim().split(/\s+/).filter(Boolean).slice(0, 2).map(part => part.charAt(0)).join("").toUpperCase() || "G";
  }

  function rankingAvatar(row, modifier) {
    const classes = "ranking-avatar " + String(modifier || "");
    const profile = typeof window.GOMO_MEMBER_PROFILE === "function" ? window.GOMO_MEMBER_PROFILE(row?.name) : null;
    const photo = safeRankingPhoto(row?.photo) || safeRankingPhoto(profile?.photo);
    if (photo) return '<img class="' + classes + '" src="' + esc(photo) + '" alt="' + esc(row?.name || "") + '">';
    return '<span class="' + classes + '" aria-hidden="true">' + esc(rankingInitials(row?.name)) + '</span>';
  }
  ...
  function renderRankingPodium(rows) {
    ...
    const order = [1, 0, 2].filter(index => index < rows.length);
    const cards = order.map(index => {
      const row = rows[index];
      return '<div class="ranking-podium__item ranking-podium__item--' + variants[index] + '">' +
        '<span class="ranking-podium__place">' + medals[index] + '</span>' +
        rankingAvatar(row, "ranking-podium__avatar") +
        '<strong class="ranking-podium__name" title="' + esc(row.name) + '">' + esc(row.name) + '</strong>' +
        '<small class="ranking-podium__role">' + esc(row.role || "—") + '</small>' +
        '<strong class="ranking-podium__score">' + esc(formatNumber(row.value)) + '</strong>' +
      '</div>';
    }).join("");
    return '<div class="ranking-podium">' + cards + '</div>';
  }

  function renderRankingRow(row, place) {
    return '<li class="ranking-row">' +
      '<span class="ranking-place">' + esc(place) + '</span>' +
      rankingAvatar(row, "ranking-row__avatar") +
      '<span class="ranking-name"><strong title="' + esc(row.name) + '">' + esc(row.name) + '</strong><small>' + esc(row.role || "—") + '</small></span>' +
      '<span class="ranking-score">' + esc(formatNumber(row.value)) + '</span>' +
    '</li>';
  }

  function renderRankingCelebration(rows) {
    if (!rows.length) return "";
    const message = RANKING_CELEBRATION[lang()] || RANKING_CELEBRATION.fr;
    return '<div class="ranking-celebration">' +
      '<img src="/r5fapper/images/r5fapper-victoire.png?v=20.11" alt="">' +
      '<p>' + esc(message) + '</p>' +
    '</div>';
  }

  function renderCategory(icon, title, rows) {
    const x = t();
    const celebration = renderRankingCelebration(rows);
    const podium = renderRankingPodium(rows.slice(0, 3));
    const remaining = rows.slice(3);
    const list = remaining.length
      ? remaining.map((row, index) => renderRankingRow(row, index + 4)).join("")
      : (!rows.length ? '<li class="ranking-row ranking-row--empty"><strong>' + esc(x.noRows) + '</strong></li>' : "");
    const listMarkup = list ? '<ol class="ranking-list">' + list + '</ol>' : "";
    return '<article class="ranking-card"><div class="ranking-card__head"><h2>' + icon + ' ' + esc(title) + '</h2></div>' + celebration + podium + listMarkup + '</article>';
  }
`;
}
...
function upgradeRankingsPatch(source) {
  ...
  patch = patch
    .replace(
      'const prev = totals.get(id) || {name:String(row.name || known?.name || ""),role:String(known?.role || row.role || ""),value:0};',
      'const prev = totals.get(id) || {name:String(row.name || known?.name || ""),role:String(known?.role || row.role || ""),photo:String(known?.photo || row.photo || ""),value:0};'
    )
    .replace(
      'if (!prev.role) prev.role = String(known?.role || "");',
      'if (!prev.role) prev.role = String(known?.role || "");\n        if (!prev.photo) prev.photo = String(known?.photo || row.photo || "");'
    )
    .replace(
      'return [...totals.values()].filter(row => row.value > 0).sort((a,b) => b.value - a.value || a.name.localeCompare(b.name, lang())).slice(0, 10);',
      'return [...totals.values()].filter(row => row.value > 0).sort((a,b) => b.value - a.value || a.name.localeCompare(b.name, lang()));'
    );

  const rendererStart = patch.indexOf("  function renderCategory(icon, title, rows) {");
  const rendererEnd = patch.indexOf("  async function loadRankings() {", rendererStart);
  if (rendererStart >= 0 && rendererEnd > rendererStart) {
    patch = patch.slice(0, rendererStart) + rankingsRendererUpgrade() + "\n" + patch.slice(rendererEnd);
  }
